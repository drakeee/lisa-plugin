...
{
	int modelid;
	Vector4 position; //NOTE: We can store vehicle position and Z rotation in one vector
	int color1, color2;

	CArgReader argReader(L);
	argReader.ReadNumber(modelid);
	argReader.ReadVector4D(position);
	...
	sampgdk::logprintf("vehicle data: %d | %f %f %f | %f | %d %d", modelid, position.x, position.y, position.z, position.w, color1, color2);
